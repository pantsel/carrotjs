...
class Server {
  ...
  async setupExchanges() {
    ...
    logger.debug("----------------------------------------------------------")
    logger.debug("Asserting exchanges in existence");
    logger.debug("----------------------------------------------------------")
    ...
    for(let key in exchanges) {
      ...
      let bindings = exchanges[key].bindings;
  ...
  async setupQueues() {
    ...
    logger.debug("----------------------------------------------------------")
    logger.debug("Asserting orphaned queues in existence");
    logger.debug("----------------------------------------------------------")
  ...
  async setupProcedures() {
    ...
    for (let key in procedures) {
      if (!procedures[key].handler) {
        logger.warn(`No handler defined for procedure "${key}"`)
      } else {
        let handlerFile = require(path.join(process.cwd(),procedures[key].handler.path));
        let method = procedures[key].handler.method;

        this.register(key, procedures[key].options || {}, handlerFile[method]);
      }
    }
  }
