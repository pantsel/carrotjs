...
class Server {
  ...
  async connect() {
    ...
    this.conn.on('error', async function (err) {
      logger.error("Connection error %s", err)
    })
    this.conn.on('blocked', function () {
      logger.warn("Connection blocked")
    })
    this.conn.on('unblocked', function () {
     ...
     */
    this.ch.on('close', async function () {
      return self.reconnect();
    })
    this.ch.on('error', async function () {

    })
    this.ch.on('return', this.onReturn)
    this.ch.on('drain', this.onDrain)
  ...
  reconnect() {
    let self = this;
    if(this.config.reconnectOnChannelClose) {
