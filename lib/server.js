...
class Server {
  ...
  async registerSubscriptions() {
    ...
    for(let key in subs) {
      ...
      this.subscribe(queue_name, this.toMethod(handlerFile, handlerMethods, options));
  ...
  async registerProcedures() {
    ...
    for (let key in procedures) {
      ...
      if (!handler) {
        ...
        this.register(key, procedures[key].options || {}, this.toMethod(handlerFile, handlerMethods));
      }
    }
  }
