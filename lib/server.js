...
class Server {
  ...
  includeConfig() {
    ...
    elligibleIncludes.forEach(entity => {
  ...
  configQueues() {
    ...
    for(let key in this.config.queues) {
      this.checkQueue(key, function (err, ok) {
        if(err) {
          logger.error("Queue `%s` does not exist", key);
          logger.debug("Creating queue => %s", key);
          self.ch.assertQueue(key, self.config.queues.options || {
            exclusive: false,
            durable: true,
            autoDelete: false
          })
        }else{
          logger.debug("Queue `%s` exists",key, JSON.stringify(ok));
        }
